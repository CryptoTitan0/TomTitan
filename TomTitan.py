                    ...
                    async with session.post(url=url, headers=headers, data=data, ssl=False) as response:
                        ...
                        if 'status' in play_game:
                            if play_game['status'] == 0:
                                self.print_timestamp(
                                    f"{Fore.BLUE + Style.BRIGHT}[ Game Started ][Subscribe Here https://youtube.com/@crypto_titan1]{Style.RESET_ALL}"
                                    f"{Fore.WHITE + Style.BRIGHT} | {Style.RESET_ALL}"
                                    f"{Fore.YELLOW + Style.BRIGHT}[ Please Wait ~30 Seconds ]{Style.RESET_ALL}"
                                )
                                sleep(30 + random.randint(3, 5))
                                await self.claim_game(token=token, points=random.randint(370, 400))
...
╚══██╔══╝██║╚══██╔══╝██╔══██╗████╗  ██║
   ...
   ╚═╝   ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═══╝
        ...
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}[ Join Our Channel: https://t.me/Crypto_Titan0 ]{Style.RESET_ALL}")

        account_files = [f for f in os.listdir() if f.startswith('accounts-') and f.endswith('.json')]
        account_files.sort(key=lambda x: int(re.findall(r'\d+', x)[0]))
        
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}╔═══════════════════════════════════════════╗{Style.RESET_ALL}")
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}║       Select an option [MADE BY Crypto_Titan0]       ║{Style.RESET_ALL}")
        tomarket.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}╠═══════════════════════════════════════════╣{Style.RESET_ALL}")
